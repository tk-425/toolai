...
describe('skills adapter', () => {
  it('does not exclude previously global skill names from bundle detection', () => {
    const aliases = new Map([
      ['discuss', 'team-pack/discuss'],
      ['web-fetch', 'team-pack/web-fetch']
    ])

    expect(detectBundles(aliases)).toEqual(
      new Map([
        ['team-pack', ['discuss', 'web-fetch']]
      ])
    )
  })
